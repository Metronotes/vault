...
func newStaticUserPassword(ctx context.Context, dbi *dbPluginInstance, generator passwordGenerator, passwordPolicy string) (password string, err error) {
	...
	if dbi.database.legacyDatabase != nil {
		password, err := dbi.database.legacyDatabase.GenerateCredentials(ctx)
		if err != nil {
			return "", err
		}
		return password, nil
	}
	password, err = generatePassword(ctx, generator, passwordPolicy)
	if err != nil {
		return "", err
	}
	return password, nil
